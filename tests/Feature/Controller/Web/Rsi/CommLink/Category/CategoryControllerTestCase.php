...
class CategoryControllerTestCase extends UserTestCase
{
    ...
    /**
     * @covers \App\Http\Controllers\Web\Rsi\CommLink\Category\CategoryController::index
     */
    public function testIndex()
    {
        $response = $this->actingAs($this->user)->get(route('web.rsi.comm-links.categories.index'));

        $response->assertStatus(static::RESPONSE_STATUSES['index']);
        if ($response->status() === Response::HTTP_OK) {
            $response->assertViewIs('web.rsi.comm_links.categories.index')->assertSee($this->category->name);
        }
    }

    /**
     * @covers \App\Http\Controllers\Web\Rsi\CommLink\Category\CategoryController::show
     * @covers \App\Models\Rsi\CommLink\Category\Category
     */
    public function testShow()
    {
        $response = $this->actingAs($this->user)->get(
            route('web.rsi.comm-links.categories.show', $this->category)
        );

        $response->assertStatus(static::RESPONSE_STATUSES['show']);
        if ($response->status() === Response::HTTP_OK) {
            $response->assertViewIs('web.rsi.comm_links.index')->assertSee(
