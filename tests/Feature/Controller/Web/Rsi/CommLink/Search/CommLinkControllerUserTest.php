...
class CommLinkControllerUserTest extends CommLinkSearchControllerTestCase
{
    ...
    /**
     * @covers \App\Http\Controllers\Web\Rsi\CommLink\CommLinkSearchController::searchByTitle
     */
    public function testSearch(): void
    {
        ...
        $response = $this->actingAs($this->user)->post(
            route('web.rsi.comm-links.search-by-title.post'),
            [
                'keyword' => 'Example',
            ]
        );
        ...
        if ($response->status() === Response::HTTP_OK) {
            $response->assertViewIs('web.rsi.comm_links.index');
        }
    }

    /**
     * @covers \App\Http\Controllers\Web\Rsi\CommLink\CommLinkSearchController::search
     */
    public function testSearchView(): void
    {
        $response = $this->actingAs($this->user)->get(route('web.rsi.comm-links.search'));

        $response->assertStatus(static::RESPONSE_STATUSES['search']);

        if ($response->status() === Response::HTTP_OK) {
            $response->assertViewIs('web.rsi.comm_links.search');
        }
    }
