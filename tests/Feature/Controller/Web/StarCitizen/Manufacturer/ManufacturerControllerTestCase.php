...
class ManufacturerControllerTestCase extends StarCitizenTestCase
{
     ...
     * Test Index
     *
     * @covers \App\Http\Controllers\Web\StarCitizen\Manufacturer\ManufacturerController::index
     * @covers \App\Http\Controllers\Api\V1\StarCitizen\Manufacturer\ManufacturerController::index
     */
    public function testIndex()
    {
        $response = $this->actingAs($this->user)->get(route('web.starcitizen.manufacturers.index'));
        $response->assertStatus(static::RESPONSE_STATUSES['index']);

        if ($response->status() === Response::HTTP_OK) {
            $response->assertViewIs('web.starcitizen.manufacturers.index')
    ...
    public function testEdit()
    {
        ...
        $response = $this->actingAs($this->user)->get(
            route('web.starcitizen.manufacturers.edit', $manufacturer->getRouteKey())
        );
        $response->assertStatus(static::RESPONSE_STATUSES['edit']);

        if ($response->status() === Response::HTTP_OK) {
            $response->assertViewIs('web.starcitizen.manufacturers.edit')
                //->assertDontSee(__('Keine Hersteller vorhanden'))
                ->assertSee(__('de_DE'))
                ->assertSee(__('en_EN'))
    ...
    {
        $response = $this->actingAs($this->user)->get(
            route('web.starcitizen.manufacturers.edit', static::MODEL_ID_NOT_EXISTENT)
        );
    ...
    public function testUpdate()
    {
        ...
        $response = $this->actingAs($this->user)->patch(
            route('web.starcitizen.manufacturers.update', $manufacturer->getRouteKey()),
            [
    ...
    {
        $response = $this->actingAs($this->user)->patch(
            route('web.starcitizen.manufacturers.update', static::MODEL_ID_NOT_EXISTENT),
            [
                'known_for_en_EN' => 'Hersteller Known For',
                'description_en_EN' => 'Hersteller description',
                'known_for_de_DE' => 'Hersteller Known For Deutsch',
