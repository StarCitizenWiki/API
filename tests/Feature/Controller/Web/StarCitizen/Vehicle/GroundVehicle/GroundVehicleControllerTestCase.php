...
class GroundVehicleControllerTestCase extends StarCitizenTestCase
{
     ...
     * Test Index
     *
     * @covers \App\Http\Controllers\Web\StarCitizen\Vehicle\GroundVehicle\GroundVehicleController::index
     * @covers \App\Http\Controllers\Api\V1\StarCitizen\Vehicle\VehicleController
     */
    public function testIndex()
    {
        $response = $this->actingAs($this->user)->get(route('web.starcitizen.vehicles.ground-vehicles.index'));
        $response->assertStatus(static::RESPONSE_STATUSES['index']);

        if ($response->status() === Response::HTTP_OK) {
            $response->assertViewIs('web.starcitizen.vehicles.ground_vehicles.index')
    ...
    public function testEdit()
    {
        ...
        if ($response->status() === Response::HTTP_OK) {
            $response->assertViewIs('web.starcitizen.vehicles.ground_vehicles.edit')
    ...
    {
        $response = $this->actingAs($this->user)->get(
            route('web.starcitizen.vehicles.ground-vehicles.edit', static::MODEL_ID_NOT_EXISTENT)
        );
    ...
    public function testUpdate()
    {
        ...
        $response = $this->actingAs($this->user)->patch(
            route('web.starcitizen.vehicles.ground-vehicles.update', $groundVehicle->getRouteKey()),
            [
    ...
    public function testUpdateNotFound()
    {
        ...
        $response = $this->actingAs($this->user)->patch(
            route('web.starcitizen.vehicles.ground-vehicles.update', static::MODEL_ID_NOT_EXISTENT),
            [
                'en_EN' => 'GroundVehicle translation',
                'de_DE' => 'Translation Deutsch',
            ]
