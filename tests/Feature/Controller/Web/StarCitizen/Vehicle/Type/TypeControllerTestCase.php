...
class TypeControllerTestCase extends StarCitizenTestCase
{
     ...
     * Test Index
     *
     * @covers \App\Http\Controllers\Web\StarCitizen\Vehicle\Type\TypeController::index
     */
    public function testIndex()
    {
        $response = $this->actingAs($this->user)->get(route('web.starcitizen.vehicles.types.index'));
        $response->assertStatus(static::RESPONSE_STATUSES['index']);

        if ($response->status() === Response::HTTP_OK) {
            $response->assertViewIs('web.starcitizen.vehicles.types.index')
     ...
     * Test Edit
     *
     * @covers \App\Http\Controllers\Web\StarCitizen\Vehicle\Type\TypeController::edit
     */
    public function testEdit()
    {
        ...
        $response = $this->actingAs($this->user)->get(
            route('web.starcitizen.vehicles.types.edit', $vehicleType->getRouteKey())
        );
        $response->assertStatus(static::RESPONSE_STATUSES['edit']);

        if ($response->status() === Response::HTTP_OK) {
            $response->assertViewIs('web.starcitizen.vehicles.types.edit')
    ...
    {
        $response = $this->actingAs($this->user)->get(
            route('web.starcitizen.vehicles.types.edit', static::MODEL_ID_NOT_EXISTENT)
        );
    ...
    public function testUpdate()
    {
        ...
        $response = $this->actingAs($this->user)->patch(
            route('web.starcitizen.vehicles.types.update', $vehicleType->getRouteKey()),
            [
    ...
    {
        $response = $this->actingAs($this->user)->patch(
            route('web.starcitizen.vehicles.types.update', static::MODEL_ID_NOT_EXISTENT),
            [
                'en_EN' => 'Vehicle Type translation',
                'de_DE' => 'Translation Deutsch',
            ]
