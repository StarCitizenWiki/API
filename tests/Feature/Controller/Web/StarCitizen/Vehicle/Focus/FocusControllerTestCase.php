...
class FocusControllerTestCase extends StarCitizenTestCase
{
     ...
     * Test Index
     *
     * @covers \App\Http\Controllers\Web\StarCitizen\Vehicle\Focus\FocusController::index
     */
    public function testIndex()
    {
        $response = $this->actingAs($this->user)->get(route('web.starcitizen.vehicles.foci.index'));
        $response->assertStatus(static::RESPONSE_STATUSES['index']);

        if ($response->status() === Response::HTTP_OK) {
            $response->assertViewIs('web.starcitizen.vehicles.foci.index')
     ...
     * Test Edit
     *
     * @covers \App\Http\Controllers\Web\StarCitizen\Vehicle\Focus\FocusController::edit
     */
    public function testEdit()
    {
        ...
        $response = $this->actingAs($this->user)->get(
            route('web.starcitizen.vehicles.foci.edit', $vehicleFocus->getRouteKey())
        );
        $response->assertStatus(static::RESPONSE_STATUSES['edit']);

        if ($response->status() === Response::HTTP_OK) {
            $response->assertViewIs('web.starcitizen.vehicles.foci.edit')
    ...
    {
        $response = $this->actingAs($this->user)->get(
            route('web.starcitizen.vehicles.foci.edit', static::MODEL_ID_NOT_EXISTENT)
        );
    ...
    public function testUpdate()
    {
        ...
        $response = $this->actingAs($this->user)->patch(
            route('web.starcitizen.vehicles.foci.update', $vehicleFocus->getRouteKey()),
            [
    ...
    {
        $response = $this->actingAs($this->user)->patch(
            route('web.starcitizen.vehicles.foci.update', static::MODEL_ID_NOT_EXISTENT),
            [
                'en_EN' => 'Vehicle Focus translation',
                'de_DE' => 'Translation Deutsch',
            ]
