...
it('does not include ship-matrix data in index endpoint', function () {
    $shipMatrixVehicle = ShipMatrixVehicle::query()->create([
        'cig_id' => 54321,
        'chassis_id' => 54,
        'name' => 'Index Test Ship',
        'slug' => 'index-test-ship',
        'manufacturer_id' => $this->shipMatrixManufacturer->id,
        'production_status_id' => $this->productionStatus->id,
        'production_note_id' => $this->productionNote->id,
        'type_id' => $this->shipType->id,
    ...
    // Ensure Ship-Matrix fields are not in the index response
    $responseData = $response->json('data');
    foreach ($responseData as $item) {
        expect($item)->not->toHaveKey('id');
        expect($item)->not->toHaveKey('chassis_id');
        expect($item)->not->toHaveKey('shipmatrix_name');
        expect($item)->not->toHaveKey('foci');
        expect($item)->not->toHaveKey('msrp');
    }
});

it('does not include ship-matrix data in search endpoint', function () {
    $shipMatrixVehicle = ShipMatrixVehicle::query()->create([
        'cig_id' => 11111,
        'chassis_id' => 11,
        'name' => 'Search Test Ship',
        'slug' => 'search-test-ship',
        'manufacturer_id' => $this->shipMatrixManufacturer->id,
        'production_status_id' => $this->productionStatus->id,
        'production_note_id' => $this->productionNote->id,
        'type_id' => $this->shipType->id,
    ...
    VehicleData::query()->create([
        ...
        'shipmatrix_id' => $shipMatrixVehicle->id,
        'name' => 'Search Test',
        'class_name' => 'Search_Test',
        'data' => ['test' => 'data'],
    ]);

    $response = $this->postJson('/api/vehicles/search', ['query' => 'Search']);
    ...
    // Ensure Ship-Matrix fields are not in the search response
    $responseData = $response->json('data');
    foreach ($responseData as $item) {
        expect($item)->not->toHaveKey('id');
        expect($item)->not->toHaveKey('chassis_id');
