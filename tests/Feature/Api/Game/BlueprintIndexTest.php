...
it('includes merged resources field with unique entries from ingredients and dismantle returns', function (): void {
    $iron = Commodity::factory()->create(['name' => 'Iron']);
    $gold = Commodity::factory()->create(['name' => 'Gold']);

    BlueprintData::factory()
        ->for(Blueprint::factory(), 'blueprint')
        ->for($this->defaultVersion, 'gameVersion')
        ->withIngredients($iron, $gold)
        ->withDismantleReturns([
            ['commodity' => $iron, 'quantity_scu' => 0.01],
        ])
        ->create([
            'output_name' => 'Resource Widget',
            'data' => ['tiers' => []],
        ]);
    ...
    $resources = $response->json('data.0.resources');
    expect($resources)->toHaveCount(2);

    $resourceUuids = array_column($resources, 'resource_type_uuid');
    expect($resourceUuids)->toContain($iron->uuid, $gold->uuid);
});
