...
it('imports resource locations with starmap data links', function (): void {
    ...
    expect($location->starmapLocationData->pluck('id')->all())->toBe([$starmapLocationData->id]);
});
...
it('normalizes variant group names from source data', function (): void {
    ...
    expect(ResourceLocation::query()->whereIn('group_name', ['FPS mineables', 'Havestables'])->count())->toBe(0);
});
