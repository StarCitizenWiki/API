                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                        @foreach (data_get($group, 'missions', []) as $mission)
                            @if (data_get($mission, 'web_url'))
                                <a href="{{ data_get($mission, 'web_url') }}" class="flex items-center justify-between gap-2 rounded-lg border border-base-300 bg-base-100 px-3 py-2 transition hover:border-base-content/20 hover:bg-base-200/35">
