...
<div {{ $attributes->merge(['class' => 'card border border-base-300 bg-base-100 shadow'])}}>
    <div class="card-body gap-4">
        ...
        <dl class="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            @if ($inventoryCapacity !== null)
                <div class="space-y-1">
                    <dt class="text-xs font-medium uppercase tracking-wide text-base-content/45">Inventory Capacity</dt>
            ...
            <details class="group" open>
                <summary class="flex cursor-pointer items-center gap-2 py-2 text-sm font-semibold text-base-content/70 list-none [&::-webkit-details-marker]:hidden">
                    ...
                    <dl class="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 pt-1 pb-2">
                        @if ($resourceCapacitySCU !== null)
                            <div class="space-y-1">
                                <dt class="text-xs font-medium uppercase tracking-wide text-base-content/45">Resource Capacity</dt>
            ...
            <details class="group">
                <summary class="flex cursor-pointer items-center gap-2 py-2 text-sm font-semibold text-base-content/70 list-none [&::-webkit-details-marker]:hidden">
                    ...
                    <dl class="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 pt-1 pb-2">
                        @if ($isImmutable !== null)
                            <div class="space-y-1">
                                <dt class="text-xs font-medium uppercase tracking-wide text-base-content/45">Immutable</dt>
                                <dd class="text-sm font-semibold text-base-content">{{ $isImmutable ? 'Yes' : 'No' }}</dd>
                            </div>
                        @endif
                        @if ($isExternalContainer !== null)
                            <div class="space-y-1">
                                <dt class="text-xs font-medium uppercase tracking-wide text-base-content/45">External Container</dt>
                                <dd class="text-sm font-semibold text-base-content">{{ $isExternalContainer ? 'Yes' : 'No' }}</dd>
                            </div>
                        @endif
                        @if ($isClosedContainer !== null)
                            <div class="space-y-1">
                                <dt class="text-xs font-medium uppercase tracking-wide text-base-content/45">Closed Container</dt>
                                <dd class="text-sm font-semibold text-base-content">{{ $isClosedContainer ? 'Yes' : 'No' }}</dd>
                            </div>
                        @endif
                        @if ($isOpenContainer !== null)
                            <div class="space-y-1">
                                <dt class="text-xs font-medium uppercase tracking-wide text-base-content/45">Open Container</dt>
                                <dd class="text-sm font-semibold text-base-content">{{ $isOpenContainer ? 'Yes' : 'No' }}</dd>
                            </div>
                        @endif
                        @if (is_array($ports) && !empty($ports))
                            <div class="space-y-1 sm:col-span-2">
                                <dt class="text-xs font-medium uppercase tracking-wide text-base-content/45">Ports</dt>
                                <dd class="text-sm">
