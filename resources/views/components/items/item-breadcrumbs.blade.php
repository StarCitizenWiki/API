...
$addBreadcrumb = function (string $label, ?string $url) use (&$breadcrumbs): void {
    $breadcrumbs[] = ['label' => $label, 'url' => $url];
};
$makeUrl = function (array $filter) use ($versionParams): string {
...
if ($item !== null) {
    ...
    if ($isShip) {
        ...
        if (in_array($normalizedType, $fpsArmorTypes, true)) {
