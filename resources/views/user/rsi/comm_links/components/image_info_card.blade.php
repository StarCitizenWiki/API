    ...
    @if(Str::contains($image->metadata->mime, 'video'))
        <div style="position: relative;" class="comm-link-card-image">
            @unless($image->tags->isEmpty())
                <a href="{{ route('web.user.rsi.comm-links.images.index-by-tag', $image->tags->first()->getRouteKey()) }}"
            ...
            @can('web.user.rsi.comm-links.view')
                <div class="btn-group d-flex mb-2" style="gap: 0.5rem">
                    ...
                    <a type="button" title="@lang('Edit') @lang('Tags')" class="btn btn-secondary w-25"
                       href="{{ route('web.user.rsi.comm-links.images.edit-tags', $image->getRouteKey()) }}">
    ...
    @if(@isset($loop))
        <div class="image-info-card-bottom list-group list-group-flush collapse mt-1"
                ...
                <div class="tag-container">
                    @foreach($image->tags as $tag)
                        <a class="badge badge-secondary m-0"
                           href="{{ route('web.user.rsi.comm-links.images.index-by-tag', $tag->getRouteKey()) }}"
            ...
            <ul class="list-unstyled mb-0">
                ...
                <li><a class="url"
                       href="{{ route('web.user.rsi.comm-links.images.show', $image->getRouteKey()) }}">@lang('Dateiinfo')</a>
                </li>
                @if(Str::contains($image->metadata->mime, 'image') || Str::contains($image->metadata->mime, 'video'))
                    <li><a class="url"
                           href="{{ route('web.user.rsi.comm-links.images.similar', $image->getRouteKey()) }}">@lang('Ähnliche Dateien (alpha)')</a>
                    </li>
                @endif
            </ul>
                ...
                <div>
                    ...
                    <ul class="list-unstyled mb-0">
                        @foreach($image->duplicates as $duplicate)
                            <li><a class="url"
                                   href="{{ route('web.user.rsi.comm-links.images.show', $duplicate->getRouteKey()) }}">{{ $duplicate->name }}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
