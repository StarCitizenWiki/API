...
@section('content')
    <div class="d-flex mb-3">
        @unless(null === $prev)
            <a href="{{ route('web.transcripts.edit', $prev) }}" class="btn btn-outline-secondary">@lang('Vorheriges')</a>
        @else
            <a href="#" class="btn btn-outline-secondary disabled">@lang('Vorheriges')</a>
        @endunless
        @unless(null === $next)
            <a href="{{ route('web.transcripts.edit', $next) }}" class="btn btn-outline-secondary ml-auto">@lang('Nächstes')</a>
        @else
    ...
    @component('components.forms.form', [
        'method' => 'PATCH',
        'action' => route('web.transcripts.update', $transcript->getRouteKey()),
        'class' => 'card mb-3',
    ])
        <div class="wrapper">
