...
@section('content')
    @include('components.comm_links.create_tag_form')
            ...
            <table class="table table-striped mb-0" data-order='[[ 2, "desc" ]]' data-page-length="50" data-length-menu='[ [25, 50, 100, -1], [25, 50, 100, "@lang('Alle')"] ]'>
                ...
                @forelse($tags as $tag)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('web.rsi.comm-links.images.index-by-tag', $tag->getRouteKey()) }}">{{ $tag->name }}</a>
                        </td>
                        <td>
                            {{ $tag->name_en }}
                        </td>
                        ...
                        <td class="text-center">
                            @component('components.edit_delete_block')
                                @can('web.rsi.comm-links.update')
                                    @slot('edit_url')
                                        {{ route('web.rsi.comm-links.image-tags.edit', $tag->getRouteKey()) }}
                                    @endslot
                                @endcan
                                {{ $tag->getRouteKey() }}
                            @endcomponent
