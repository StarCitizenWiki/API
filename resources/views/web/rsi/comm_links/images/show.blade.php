            ...
            <table class="table mb-0 table-responsive">
                    ...
                    <td>
                        <ul class="list-unstyled mb-0">
                            ...
                            @if(\Illuminate\Support\Str::contains($image->metadata->mime, 'image'))
                                <li>
                                    <a class="url" href="{{ route('web.rsi.comm-links.images.similar', $image->getRouteKey()) }}">
                            ...
                            @can('web.rsi.comm-links.view')
                                ...
                                <li>
                                    <a class="url" title="@lang('Edit') @lang('Tags')" href="{{ route('web.rsi.comm-links.images.edit-tags', $image->getRouteKey()) }}">
                                        @lang('Edit') @lang('Tags')
                                    </a>
                                </li>
                            @endcan
                ...
                <tr>
                    ...
                    <td>
                        @forelse($image->tags as $tag)
                            <a class="badge badge-secondary m-0" href="{{ route('web.rsi.comm-links.images.index-by-tag', $tag->getRouteKey()) }}" title="{{ $tag->images_count }} @lang('Bilder mit diesem Tag')">
                                {{ $tag->translated_name }}
                            </a>
                        @empty
                            @lang('None')
                ...
                <tr>
                    ...
                    <td>
                        ...
                        @forelse($image->duplicates as $duplicate)
                            <li><a class="url" href="{{ route('web.rsi.comm-links.images.show', $duplicate->getRouteKey()) }}">{{ $duplicate->name }}</a></li>
                        @empty
                            <li>@lang('Keine')</li>
                        @endforelse
                        </ul>
                ...
                @else
                    <tr>
                        ...
                        <td>
                            <a class="url" href="{{ route('web.rsi.comm-links.images.show', $image->baseImage->getRouteKey()) }}">{{ $image->baseImage->name }}</a>
                        </td>
                    </tr>
                @endif
                <tr>
                    ...
                    <td>
                        ...
                        @forelse($image->commLinks->sortByDesc('cig_id')->take(5) as $commLink)
                            <li>
                                <a href="{{ route('web.rsi.comm-links.show', $commLink->getRouteKey()) }}"
