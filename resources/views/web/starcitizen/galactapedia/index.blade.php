                ...
                <tbody>
                    @forelse($articles as $article)
                        <tr>
                            ...
                            <td>
                                {{ optional($article->english())->translation ? __('Ja') : __('Nein') }}
                            </td>
                            <td class="text-{{ $article->in_wiki ? 'success' : 'danger' }}">
                                {{ $article->in_wiki ? __('Ja') : __('Nein') }}
                            </td>
                            ...
                            <td class="text-center">
                                @component('components.edit_delete_block')
                                    @slot('show_url')
                                        {{ route('web.starcitizen.galactapedia.show', $article->getRouteKey()) }}
                                    @endslot
                                    @can('web.starcitizen.galactapedia.update')
                                        @slot('edit_url')
                                            {{ route('web.starcitizen.galactapedia.edit', $article->getRouteKey()) }}
                                        @endslot
                                    @endcan
                                    {{ $article->getRouteKey() }}
                                @endcomponent
