...
@section('content')
    <div class="d-flex mb-3 nav-bar">
        @unless(null === $prev)
            <a href="{{ route('web.starcitizen.galactapedia.show', $prev) }}" class="btn btn-outline-secondary">@lang('Vorheriger')</a>
        @else
            <a href="#" class="btn btn-outline-secondary disabled">@lang('Vorheriger')</a>
        @endunless
        @unless(null === $next)
            <a href="{{ route('web.starcitizen.galactapedia.show', $next) }}" class="btn btn-outline-secondary ml-auto">@lang('Nächster')</a>
        @else
                        ...
                        <tr>
                            ...
                            <td>
                                @forelse($article->related as $related)
                                    <a href="{{ route('web.starcitizen.galactapedia.show', $related->getRouteKey()) }}" style="word-break: break-all">
                                        {{ $related->title }}
                                    </a>
                                    <br>
                                    @empty
