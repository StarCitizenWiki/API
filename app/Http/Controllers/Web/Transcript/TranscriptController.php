...
namespace App\Http\Controllers\Web\Transcript;
...
{
    private const TRANSCRIPT_UPDATE_PERMISSION = 'web.transcripts.update';
    ...
    {
        $this->authorize('web.transcripts.index');

        $transcripts = Transcript::query()->orderBy('id')->paginate(500);

        return view(
            'web.transcripts.index',
            [
    ...
    {
        $this->authorize('web.transcripts.view');

        return view(
            'web.transcripts.show',
            [
    ...
    public function update(TranscriptUpdateRequest $request, Transcript $transcript): RedirectResponse
    {
        ...
        return redirect()->route('web.transcripts.edit', $transcript->getRouteKey())->withMessages(
            [
                'success' => [
                    $message,
                ],
