...
class VehicleResourceV3 extends AbstractBaseResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        if (in_array('ports', $includes, true)) {
            if ($request->has('filter') && isset($request->get('filter')['ports'])) {
                /** @var HasMany $hardpoints */
                $hardpoints = $this->hardpointsWithoutParent();

                $filters = collect(explode(',', $request->get('filter')['ports']))
        ...
        $data = [
            ...
            $this->mergeWhen(in_array('ports', $includes, true), [
                'ports' => HardpointResourceV3::collection($hardpoints),
            ]),
